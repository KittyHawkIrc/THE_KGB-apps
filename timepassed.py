...
if __name__ == "__main__":
	...
	if callback(api) != 'TOOSHORT':
		exit(3)
	api.locker.time[api.outgoing_channel] = datetime.timedelta(seconds=api.locker.time[api.outgoing_channel].total_seconds() - 610)
	if '0 hours and 10 minutes' not in callback(api):
		exit(4)
	api.locker.time[api.outgoing_channel] = datetime.timedelta(seconds=api.locker.time[api.outgoing_channel].total_seconds() - 6310)
